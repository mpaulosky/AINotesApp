...
public class BackfillTagsIntegrationTests : IAsyncLifetime
{
	...
	public async Task BackfillTags_Processes_Multiple_Users_Isolated()
	{
		...
		var userA = "userA";
		var userB = "userB";
		var noteA1 = new Note { Id = Guid.NewGuid(), UserId = userA, Title = "A1", Content = "A1 content", Tags = null };
		var noteA2 = new Note { Id = Guid.NewGuid(), UserId = userA, Title = "A2", Content = "A2 content", Tags = null };
		var noteB1 = new Note { Id = Guid.NewGuid(), UserId = userB, Title = "B1", Content = "B1 content", Tags = null };
		...
		// Act
		var resultA = await _handler.Handle(new BackfillTagsCommand { UserId = userA }, CancellationToken.None);
		var resultB = await _handler.Handle(new BackfillTagsCommand { UserId = userB }, CancellationToken.None);
	...
	public async Task BackfillTags_Saves_Periodically()
	{
		...
		for (int i = 1; i <= 7; i++)
		{
			_context.Notes.Add(new Note { Id = Guid.NewGuid(), UserId = userId, Title = $"Note {i}", Content = $"Content {i}", Tags = null });
		}
		await _context.SaveChangesAsync();
		_aiService.GenerateTagsAsync(Arg.Any<string>(), Arg.Any<string>(), Arg.Any<CancellationToken>())
		...
		// Act
		var result = await _handler.Handle(new BackfillTagsCommand { UserId = userId }, CancellationToken.None);

		// Assert
		result.ProcessedCount.Should().Be(7);
		result.TotalNotes.Should().Be(7);
		result.Errors.Should().BeEmpty();
		var tags = await _context.Notes.Where(n => n.UserId == userId).Select(n => n.Tags).ToListAsync();
